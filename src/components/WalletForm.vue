<template>
    <div style="width: 100%; max-width: 450px;">
      ...
      <form @submit.prevent="submitCrypto" style="background-color: black; border-radius: 10px; border: 1px solid gray; padding: 16px;">
        <label class="block mb-2 font-semibold">Select cryptocurrency:</label>
        <select v-model="newCrypto.id" required style="width: 100%; padding: 8px; background: dimgrey; color: white; border: 1px solid gray; border-radius: 5px; margin-bottom: 16px;">
        ...
        <input type="number" v-model="newCrypto.amount" min="0.01" step="0.01" required 
          style="width: 100%; padding: 8px; background: dimgrey; color: white; border: 1px solid gray; border-radius: 5px; margin-bottom: 16px;" />
  
        <label class="block mb-2 font-semibold">Purchase price:</label>
        <input type="number" v-model="newCrypto.price" min="0.01" step="0.01" required 
          style="width: 100%; padding: 8px; background: dimgrey; color: white; border: 1px solid gray; border-radius: 5px; margin-bottom: 16px;"/>
  
        <label class="block mb-2 font-semibold">Notes:</label>
        <textarea v-model="newCrypto.notes" style="width: 100%; padding: 8px; background: dimgrey; color: white; border: 1px solid gray; border-radius: 5px; resize: none;" class="mb-4"></textarea>
  
        <button class="w-full mt-2 p-3 bg-yellow-500 text-black font-bold rounded-lg hover:bg-yellow-400">
          Add
        </button>
      </form>
    </div>
